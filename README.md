...
A Relational Database is a database that uses the relational model or architecture to implement storage, querying, and updating, while
SQL is a language used to interact with the database.
- [x] Why do tables need a `primary key`?
A primary key is the main reference for finding a distinct row on a table, the value is unique to 1 item and it makes that item
universally accessible accross the database
- [x] What is the name given to a table column that references the primary key on another table.
Foreign Key
- [x] What do we need in order to have a _many to many_ relationship between two tables.
For example we have 2 Tables, Table A and Table B, if an item on A is linked to multiple items on B
and an item on B is also linked to multiple items on A.
